...
@testset "Tree Data Transforms" begin
    ...
    for ibox in BoxDMK.leaves(tree_quad)
        @test flvals_quad[1, :, ibox] ≈ fill(6.0, size(flvals_quad, 2)) atol = 1e-10
    end
    ...
    for ibox in BoxDMK.leaves(tree_quad)
        ...
        @test gvals[1, 1, :, ibox] ≈ 2 .* coords[1, :] atol = 1e-10
        @test gvals[1, 2, :, ibox] ≈ 2 .* coords[2, :] atol = 1e-10
        @test gvals[1, 3, :, ibox] ≈ 2 .* coords[3, :] atol = 1e-10

        @test hvals[1, 1, :, ibox] ≈ fill(2.0, size(hvals, 3)) atol = 1e-10
        @test hvals[1, 2, :, ibox] ≈ fill(2.0, size(hvals, 3)) atol = 1e-10
        @test hvals[1, 3, :, ibox] ≈ fill(2.0, size(hvals, 3)) atol = 1e-10
        @test hvals[1, 4, :, ibox] ≈ fill(0.0, size(hvals, 3)) atol = 1e-10
        @test hvals[1, 5, :, ibox] ≈ fill(0.0, size(hvals, 3)) atol = 1e-10
        @test hvals[1, 6, :, ibox] ≈ fill(0.0, size(hvals, 3)) atol = 1e-10
    end
    ...
    for ibox in BoxDMK.leaves(tree_quartic)
        @test fl2vals_quartic[1, :, ibox] ≈ fill(72.0, size(fl2vals_quartic, 2)) atol = 1e-9
    end
    ...
    for ibox in BoxDMK.leaves(tree_quartic)
        ...
        @test pot[1, :, ibox] ≈ expected atol = 1e-12
    end
